...
/**
 * This class filters a list of properties by their type(Bedrooms).
 */
public class BedroomsFilter implements Filtro{
    ...
    public List<Announcement> roomsFilter(List<Announcement> properties, int number) {
        ...
        for (Announcement property : properties) {
            if (Property.TypeOfProperty.HOUSE == property.getRequest().getProperty().getTypeOfProperty() && ((Home) property.getRequest().getProperty()).getNumberOfBedrooms() == number) {//melhor instance of????
                rooms.add(property);
            }
            if (Property.TypeOfProperty.APARTMENT == property.getRequest().getProperty().getTypeOfProperty() && ((Home) property.getRequest().getProperty()).getNumberOfBedrooms() == number) {
                rooms.add(property);
            }
        }
        return rooms;
